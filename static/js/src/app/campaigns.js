        ...
        preConfirm: function () {
            return new Promise(function (resolve, reject) {
                ...
                campaign = {
                    ...
                    long_term: $("#long_term").prop("checked"),
                    groups: groups,
                }
                // Submit the campaign
...
function sendTestEmail() {
    ...
    api.send_test_email(test_email_request)
        .success(function (data) {
            ...
            <i class=\"fa fa-check-circle\"></i> 测试投递已发送。</div>")
...
function dismiss() {$("#long_term").prop("checked", false);
    $("#modal\\.flashes").empty();
    $("#name").val("");
    $("#template").val("").change();
    $("#page").val("").change();
    $("#url").val("");
    $("#delivery_channel_smtp").prop("checked", true);
...
function selectedDeliveryChannel() {
    return $("input[name=delivery_channel]:checked").val() || "SMTP"
}

function updateProfileOptions(selectedProfileId) {
    var channel = selectedDeliveryChannel()
    var filteredProfiles = $.grep(sendingProfiles, function (profile) {
        var interfaceType = profile.interface_type || "SMTP"
        return interfaceType == channel
    })
    var profileSelect = $("#profile.form-control")
    if (profileSelect.data("select2")) {
        profileSelect.select2("destroy")
    }
    profileSelect.empty().append("<option></option>")
    if (filteredProfiles.length == 0) {
        profileSelect.select2({
            placeholder: channel == "DingTalkWorkNotice" ? "没有可用的钉钉一对一发信配置" : "没有可用的邮件 SMTP 发信配置",
            data: [],
        })
        return
    }
    var profile_s2 = $.map(filteredProfiles, function (obj) {
        return {
            id: obj.id,
            text: obj.name,
        }
    ...
    var selected = selectedProfileId || profile_s2[0].id
    profileSelect.val(selected.toString())
    profileSelect.trigger('change.select2')
}
    ...
    api.campaignId.get(campaigns[idx].id)
        .success(function (campaign) {
            ...
            if (!campaign.smtp.id) {
                ...
                var channel = campaign.smtp.interface_type || "SMTP"
                $("input[name=delivery_channel][value='" + channel + "']").prop("checked", true)
                updateProfileOptions(campaign.smtp.id)
            }
...
$(document).ready(function () {
    ...
    $("input[name=delivery_channel]").on("change", function () {
        updateProfileOptions()
    })
            ...
            if (campaigns.length > 0) {
                ...
                $.each(campaigns, function (i, campaign) {
                    ...
                    if (moment(campaign.launch_date).isAfter(moment())) {
                        launchDate = "计划启动时间：" + moment(campaign.launch_date).format('MMMM Do YYYY, h:mm:ss a')
                        var quickStats = launchDate + "<br><br>" + "目标人数：" + campaign.stats.total
                    } else {
                        launchDate = "启动时间：" + moment(campaign.launch_date).format('MMMM Do YYYY, h:mm:ss a')
                        var quickStats = launchDate + "<br><br>" + "目标人数：" + campaign.stats.total + "<br><br>" + "邮件打开：" + campaign.stats.opened + "<br><br>" + "点击链接：" + campaign.stats.clicked + "<br><br>" + "提交数据：" + campaign.stats.submitted_data + "<br><br>" + "错误：" + campaign.stats.error + "<br><br>" + "上报：" + campaign.stats.email_reported
                    }
