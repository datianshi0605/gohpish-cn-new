...
function sendTestEmail() {
    ...
    api.send_test_email(test_email_request)
	    ...
	    <i class=\"fa fa-check-circle\"></i> 测试邮件已发送。</div>")
...
function save(idx) {
    ...
    profile.dingtalk_app_key = $("#dingtalk_app_key").val()
    profile.dingtalk_app_secret = $("#dingtalk_app_secret").val()
    profile.dingtalk_agent_id = $("#dingtalk_agent_id").val()
    profile.dingtalk_recipient_mode = $("input[name=dingtalk_recipient_mode]:checked").val()
    profile.dingtalk_user_mappings = $("#dingtalk_user_mappings").val()
    profile.dingtalk_message_template = $("#dingtalk_message_template").val()
    profile.dingtalk_include_qr_code = $("#dingtalk_include_qr_code").prop("checked")
    if (idx != -1) {
        profile.id = profiles[idx].id
        api.SMTPId.put(profile)
            .success(function (data) {
                successFlash("发信配置已更新。")
                load()
                dismiss()
            })
            .error(function (data) {
...
function dismiss() {
    ...
    $("#dingtalk_app_key").val("")
    $("#dingtalk_app_secret").val("")
    $("#dingtalk_agent_id").val("")
    $("#dingtalk_recipient_mode_email").prop("checked", true)
    $("#dingtalk_user_mappings").val("")
    $("#dingtalk_message_template").val("您好 {{.FirstName}}，请完成本次安全演练信息核验：\n\n{{.URL}}")
    $("#dingtalk_include_qr_code").prop("checked", false)
...
function edit(idx) {
    ...
    if (idx != -1) {
        ...
        $("#dingtalk_app_key").val(profile.dingtalk_app_key)
        $("#dingtalk_app_secret").val(profile.dingtalk_app_secret)
        $("#dingtalk_agent_id").val(profile.dingtalk_agent_id)
        $("input[name=dingtalk_recipient_mode][value='" + (profile.dingtalk_recipient_mode || "email_lookup") + "']").prop("checked", true)
        $("#dingtalk_user_mappings").val(profile.dingtalk_user_mappings)
        $("#dingtalk_message_template").val(profile.dingtalk_message_template || "您好 {{.FirstName}}，请完成本次安全演练信息核验：\n\n{{.URL}}")
        $("#dingtalk_include_qr_code").prop("checked", profile.dingtalk_include_qr_code)
        $.each(profile.headers, function (i, record) {
            addCustomHeader(record.key, record.value)
        });
    } else {
        $("#profileModalLabel").text("新建发信配置")
        $("#dingtalk_message_template").val("您好 {{.FirstName}}，请完成本次安全演练信息核验：\n\n{{.URL}}")
    }
    updateInterfaceFields()
}
...
function updateInterfaceFields() {
    ...
    if (interfaceType == "DingTalkWorkNotice") {
        $("#smtpProfileFields").hide()
        $("#smtpExtraFields").hide()
        $("#dingtalkProfileFields").show()
    } else {
        $("#smtpProfileFields").show()
        $("#smtpExtraFields").show()
        $("#dingtalkProfileFields").hide()
    }
}
            ...
            if (profiles.length > 0) {
                ...
                $.each(profiles, function (i, profile) {
                    profileRows.push([
                        ...
                        "<div class='pull-right'><span data-toggle='modal' data-backdrop='static' data-target='#modal'><button class='btn btn-primary' data-toggle='tooltip' data-placement='left' title='编辑发信配置' onclick='edit(" + i + ")'>\
                    <i class='fa fa-pencil'></i>\
                    </button></span>\
		    <span data-toggle='modal' data-target='#modal'><button class='btn btn-primary' data-toggle='tooltip' data-placement='left' title='复制发信配置' onclick='copy(" + i + ")'>\
                    <i class='fa fa-copy'></i>\
                    </button></span>\
                    <button class='btn btn-danger' data-toggle='tooltip' data-placement='left' title='删除发信配置' onclick='deleteProfile(" + i + ")'>\
...
$(document).ready(function () {
    ...
    $("#interface_type").on("change", updateInterfaceFields)
    updateInterfaceFields()
    load()
})
