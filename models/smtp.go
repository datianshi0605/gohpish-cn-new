...
type SMTP struct {
	...
	DingTalkAppKey   string    `json:"dingtalk_app_key" gorm:"column:dingtalk_app_key"`
	DingTalkAppSecret string   `json:"dingtalk_app_secret" gorm:"column:dingtalk_app_secret"`
	DingTalkAgentID  string    `json:"dingtalk_agent_id" gorm:"column:dingtalk_agent_id"`
	DingTalkRecipientMode string `json:"dingtalk_recipient_mode" gorm:"column:dingtalk_recipient_mode"`
	DingTalkUserMappings string `json:"dingtalk_user_mappings" gorm:"column:dingtalk_user_mappings"`
	DingTalkMessageTemplate string `json:"dingtalk_message_template" gorm:"column:dingtalk_message_template"`
	DingTalkIncludeQRCode bool `json:"dingtalk_include_qr_code" gorm:"column:dingtalk_include_qr_code"`
...
var ErrDingTalkAppKeyNotSpecified = errors.New("No DingTalk AppKey specified")

var ErrDingTalkAppSecretNotSpecified = errors.New("No DingTalk AppSecret specified")

var ErrDingTalkAgentIDNotSpecified = errors.New("No DingTalk AgentId specified")

var ErrDingTalkRecipientModeNotSpecified = errors.New("No DingTalk recipient mode specified")

var ErrDingTalkUserMappingsNotSpecified = errors.New("No DingTalk user mappings specified")

var ErrUnsupportedSendingInterface = errors.New("Unsupported sending interface")
...
func (s *SMTP) Validate() error {
	...
	if s.Interface == "DingTalkWorkNotice" {
		if s.DingTalkAppKey == "" {
			return ErrDingTalkAppKeyNotSpecified
		}
		if s.DingTalkAppSecret == "" {
			return ErrDingTalkAppSecretNotSpecified
		}
		if s.DingTalkAgentID == "" {
			return ErrDingTalkAgentIDNotSpecified
		}
		if s.DingTalkRecipientMode == "" {
			return ErrDingTalkRecipientModeNotSpecified
		}
		if s.DingTalkRecipientMode == "mapping" && strings.TrimSpace(s.DingTalkUserMappings) == "" {
			return ErrDingTalkUserMappingsNotSpecified
		}
		return nil
	}
	if s.Interface != "SMTP" {
		return ErrUnsupportedSendingInterface
	}
	switch {
	case s.FromAddress == "":
