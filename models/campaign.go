...
func PostCampaign(c *Campaign, uid int64) error {
	...
	if c.LongTerm {
		...
		for _, g := range c.Groups {
			...
			if err := tx.Create(&CampaignAutoGroup{CampaignId: c.Id, GroupId: g.Id}).Error; err != nil {
				tx.Rollback()
				return err
			}
		}
...
func DeleteCampaign(id int64) error {
	...
	defer tx.Rollback()
	if err := tx.Model(&Campaign{}).Where("id=?", id).UpdateColumn("id", id).Error; err != nil {
		return err
	}
	for _, model := range []interface{}{&Result{}, &Event{}, &MailLog{}, &CampaignAutoGroup{}} {
		if err := tx.Where("campaign_id=?", id).Delete(model).Error; err != nil {
			return err
		}
	}
	if err := tx.Delete(&Campaign{Id: id}).Error; err != nil {
		return err
	}
	return tx.Commit().Error
}
...
func CompleteCampaign(id int64, uid int64) error {
	...
	if c.Status != CampaignComplete {
		if err := tx.Model(&c).Updates(map[string]interface{}{"completed_date": time.Now().UTC(), "status": CampaignComplete}).Error; err != nil {
			return err
		}
	}
	return tx.Commit().Error
