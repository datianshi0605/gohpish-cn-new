...
func (ps *PhishingServer) registerRoutes() {
	...
	router.HandleFunc("/qr", ps.QRCodeHandler)
	router.HandleFunc("/{path:.*}/qr", ps.QRCodeHandler)
...
func (ps *PhishingServer) QRCodeHandler(w http.ResponseWriter, r *http.Request) {
	...
	phishPath := strings.TrimSuffix(r.URL.Path, "/qr")
	if phishPath == "" {
		phishPath = "/"
	}
	targetQuery := r.URL.Query()
	for _, cacheParam := range []string{"random", "rand", "cache", "cachebuster", "cb", "_", "t", "ts"} {
		targetQuery.Del(cacheParam)
	}
	targetQuery.Set(models.RecipientParameter, rid)
	targetURL := url.URL{
		Scheme:   scheme,
		Host:     r.Host,
		Path:     phishPath,
	...
	png, err := qrcode.Encode(targetURL.String(), qrcode.Medium, 320)
	if err != nil {
		log.Error(err)
		http.NotFound(w, r)
		return
